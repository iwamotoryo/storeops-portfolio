...
**▶ [ブラウザで触れるデモを開く](https://demo.takken.7poke.com)** — ログイン不要。受験 → 採点 → 問題マップ → 科目・単元分析 → 弱点 → 復習 → みんなの成績 まで一通り操作できます。本番システムとはデータ・通信・サーバーが完全に分離した独立のデモで、登場する人物・成績・問題・学習履歴はすべて架空です。
...
- **触れるデモ**：[demo.takken.7poke.com](https://demo.takken.7poke.com) — ログイン不要。本番と同じ画面を、本番のデータベース・認証・APIを一切使わずに架空データだけで動かしています（本番とは別サーバー・別コンテナで、本番へは一切通信しません）
...
- 公開デモは、本番と同じアプリをデモ設定（架空データのみ）で別サーバー・別コンテナとして動かしている。本番のURL・Supabase・認証情報・受講生データは含まず、外部への通信も行わない
