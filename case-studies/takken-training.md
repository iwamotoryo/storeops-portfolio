...
**▶ [ブラウザで触れるデモを開く](https://takken-training-demo.pages.dev/demo/takken/)** — ログイン不要。受験 → 採点 → 問題マップ → 科目・単元分析 → 弱点 → 復習 → みんなの成績 まで一通り操作できます。本番システムとはコード・データ・通信のすべてが分離した独立のデモで、登場する人物・成績・問題・学習履歴はすべて架空です。
...
- **触れるデモ**：[takken-training-demo.pages.dev/demo/takken/](https://takken-training-demo.pages.dev/demo/takken/) — ログイン不要。本番のソースコード・データベース・認証・APIを一切使わず、架空データだけで主要画面を再現しています（手元ではファイルを開くだけで動作します）
- 公開先の方針：[デモの公開方法とセキュリティヘッダ](../docs/demo-hosting.md)。GitHub Pages はレスポンスヘッダを設定できず `frame-ancestors` / `X-Frame-Options` を返せないため使いません
...
- 公開デモ（`demo/takken/`）は、本番のソースを読み込まない独立した静的ページとして作成し、架空データだけで構成している。本番のURL・Supabase・認証情報・受講生データは含まず、外部への通信も行わない
- デモ側のハードニング：`fetch` 等の通信APIを1つも持たず、DOM は `createElement` / `textContent` だけで組み立てて `innerHTML` を使わない。報告・問い合わせ・アカウント変更は**画面上で成功を見せるだけ**で、送信も保存もしない。公開時は `frame-ancestors 'none'` と `connect-src 'none'` を含む CSP をレスポンスヘッダで付ける（[方針](../docs/demo-hosting.md)）
